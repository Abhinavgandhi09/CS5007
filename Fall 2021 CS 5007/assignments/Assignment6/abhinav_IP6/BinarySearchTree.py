...
class BinarySearchTree:
    ...
    def __countLeaves(self, cur_node):
        # If node has no children it is leaf
        if cur_node.getLeft() is None and cur_node.getRight() is None:
            return 1
        elif cur_node.getLeft() is not None and cur_node.getRight() is None:
            return self.__countLeaves(cur_node.getLeft())
        elif cur_node.getRight() is not None and cur_node.getLeft() is None:
            return self.__countLeaves(cur_node.getRight())
        else:
            return self.__countLeaves(cur_node.getLeft()) + self.__countLeaves(cur_node.getRight())
    ...
    def __countNonLeaves(self, cur_node):
        if cur_node.getLeft() is not None and cur_node.getRight() is None:
            return 1 + self.__countNonLeaves(cur_node.getLeft())  # Recursive call with child node as arg
        elif cur_node.getRight() is not None and cur_node.getLeft() is None:
            return 1 + self.__countNonLeaves(cur_node.getRight())  # Recursive call with child node as arg
        elif cur_node.getLeft() is not None and cur_node.getRight() is not None:
            return 1 + self.__countNonLeaves(cur_node.getLeft()) + self.__countNonLeaves(cur_node.getRight())  # Multiple recursive calls
        else:  # When node is leaf
            return 0
    ...
    def printParentNode(self):
        if self.getRoot() is None:
            ...
            # Traverse the tree level by level to print each node's parent
            nodes = [self.getRoot()]
            while len(nodes) > 0:
                cur_node = nodes.pop(0)
                print(self.__printParentNode(cur_node))

                if cur_node.getLeft() is not None:
                    nodes.append(cur_node.getLeft())
                if cur_node.getRight() is not None:
                    nodes.append(cur_node.getRight())

    def __printParentNode(self, cur_node):
        s = ""
        if cur_node.getParent() is None:
            s += str(cur_node.getVal()) + " is a root node."
        else:
            s += str(cur_node.getParent().getVal()) + " is the parent of " + str(cur_node.getVal())
        return s
