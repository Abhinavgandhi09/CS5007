...
class BinarySearchTree:
    ...
    def __countNonLeaves(self, cur_node):
        if cur_node.getLeft() is not None and cur_node.getRight() is None:
            ...
            return 1 + self.__countNonLeaves(cur_node.getLeft()) + self.__countNonLeaves(cur_node.getLeft())  # Multiple recursive calls
    ...
    def printParentNode(self):
        if self.getRoot() is None:
            ...
            while len(nodes) > 0:
                node = nodes.pop(0)
                print(self.__printParentNode(node))

                if node.getLeft() is not None:
                    nodes.append(node.getLeft())
                if node.getRight() is not None:
                    nodes.append(node.getRight())
